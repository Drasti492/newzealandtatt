...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', function(e) {
    if (navList && navList.classList.contains('show') && !navList.contains(e.target) && e.target !== navToggle) {
      closeNav();
    }
  });
  ...
  document.querySelectorAll('[data-artist]').forEach(function(el) {
    ...
    el.addEventListener('keydown', function(e) { if (e.key === 'Enter' || e.key === ' ') openArtistModal(Number(el.dataset.artist)); });
  });
...
document.querySelectorAll('.preview-video, .tattoo-card video').forEach(function(video) {
  var card = video.closest('.pro-card') || video.closest('.tattoo-card') || video.parentElement;
